...
public class EmailEditorScreen extends Screen {
    private EditBox emailTitleField;         // 邮件标题输入框
    private TextEditorWidget emailBodyField; // 使用 TextEditorWidget
    ...
    public EmailEditorScreen(NPCModel npc) {
        super(new TextComponent("Email Editor with " + npc.getNPCName()));
    ...
    protected void init() {
        ...
        // 邮件标题输入框
        this.emailTitleField = new EditBox(this.font, 20, centerY - 100, leftPanelWidth - 40, 20, new TextComponent("Enter Email Title"));
        this.addWidget(this.emailTitleField);

        // 正文输入框
        this.emailBodyField = new TextEditorWidget(20, centerY - 70, leftPanelWidth - 40, 150);
        this.addRenderableWidget(this.emailBodyField);
    ...
    private void generateAdvice() {
        String title = this.emailTitleField.getValue().trim();      // 获取邮件标题
        String body = this.emailBodyField.getText().trim();         // 获取邮件正文
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        // 渲染标题输入框
        this.emailTitleField.render(poseStack, mouseX, mouseY, partialTicks);

        // 渲染用户问题输入框
        this.userQuestionField.render(poseStack, mouseX, mouseY, partialTicks);

        // 渲染正文输入框
        this.emailBodyField.render(poseStack, mouseX, mouseY, partialTicks);

        // 渲染建议面板
        this.advicePanel.render(poseStack, mouseX, mouseY, partialTicks);

        // 绘制标题文字
        drawCenteredString(poseStack, this.font, "Email Editor with " + currentNPC.getNPCName(), this.width / 2 - 100, 5, 0xFFFFFF);
